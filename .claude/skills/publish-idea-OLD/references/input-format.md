...
| consecutive lines starting with `* ` or `- ` | `<ul><li>…</li></ul>` |
...
| `@callout[LABEL \| text]` | A light red-tinted box with a small uppercase mono label above italic body text — for quoting a source's own framework description inline (e.g. Sequoia's original slide prompts) without it reading as the author's voice. |
