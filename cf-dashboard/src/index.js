...
function buildResponsePayload(mainRows, chainRows, snippetRows, days) {
  ...
  for (const r of snippetRows) {
    const hits = Number(r.hits);
    snippetTotal += hits;
    snippetDaily[r.day] = (snippetDaily[r.day] || 0) + hits;
  }
  const snippetSeries = Object.keys(snippetDaily)
    .sort()
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .snippet-pin { border: 1px solid color-mix(in srgb, var(--snippet) 40%, transparent); border-radius: 10px; padding: 1.25rem 1.25rem 0.75rem; margin-bottom: 2rem; }
  .snippet-pin h2 { color: var(--snippet); }
  .snippet-sub { color: var(--muted); font-size: 0.78rem; margin: -0.4rem 0 0.75rem; }
</style>
</head>
<body>
...
function renderSnippetSection(sn) {
  if (!sn) return '';
  const total = sn.total;
  const daily = sn.daily || [];
  ...
  if (daily.length > 0) {
    ...
    const bars = daily.map((d, i) => {
      ...
      return '<rect x="' + (cx - barW / 2).toFixed(1) + '" y="' + (padT + innerH - h).toFixed(1)
        ...
        + '" fill="var(--snippet)" rx="2" opacity="0.85" />';
    }).join('');
    ...
    chart = '<div class="empty">No snippet-driven fetches in this window.</div>';
  }

  return '<div class="snippet-pin">'
    + '<h2>Snippet visits</h2>'
    + '<div class="snippet-sub">AI assistants that fetched llms.txt — each one is a context-snippet user whose LLM consulted the site</div>'
    + '<div class="totals"><div class="tile"><div class="n" style="color:var(--snippet)">' + total + '</div><div class="l">llms.txt fetches</div></div></div>'
    + chart
    + '</div>';
}
